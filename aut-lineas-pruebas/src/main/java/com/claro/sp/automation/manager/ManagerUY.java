...
public class ManagerUY extends Manager {
    ...
    public void initCellular(Cellular c) throws Exception {
        ...
        c.setDealerId(DEALER_UY);

        if (c.isCorporativeClient()) {
            c.setAccountId(ACCOUNT_CORPO_UY);
            c.setClientId(CLIENT_CORPO_UY);
        } else {
            c.setAccountId(ACCOUNT_UY);
            c.setClientId(CLIENT_UY);
        }
    ...
    public String createRandomNim(Cellular c) throws SQLException {
        ...
        try {
            if ("N".equalsIgnoreCase(c.getNode())) {
                if(c.isFlagRange())
                    query = String.format(QUERY_RANDOM_NIM_UY, c.getCodeArea(),c.getBeginRange(),c.getEndRange());
                else
                    query = String.format(QUERY_RANDOM_NIM_UY, c.getCodeArea(),DEFAULT_BEGIN_RANGE,DEFAULT_END_RANGE);
                rs = executeQuery(this.prodConn, query);
            }
            if (rs.next())
                return calibrateLength(rs.getString("NIM"),8);
    ...
    public void insertActivationRanges(Cellular c) throws Exception {
        String query = String.format(BLOCK_INSERT_ACTIVATION_RANGES, c.getCellularNumber(), c.getCellularNumber(), c.getCellularNumber(), c.getBlock_code(), c.getCellularNumber().substring(0, 2),
                c.getCellularNumber().substring(0, 2), c.getCellularNumber().substring(0, 2), c.getCellularNumber(), c.getCellularNumber(), c.getSubid());
